...
class OneMapClient():
    ...
    def get_token(self, email=None, password=None):
        ...
        #data_well_formated = json.dumps(fields)
        #print(data_well_formated)
        
        #response = requests.post(self.url_base + "/privateapi/auth/post/getToken",
         #                        json=data_well_formated)
         
        response = requests.post(url = self.url_base + "/privateapi/auth/post/getToken", json={"email": "email", "password": "password"})
    ...
    def generate_static_map(self, layer_chosen, location, zoom, width, height, polygons=None, lines=None, points=None, color=None, fill_color=None):
        ...
        try:
            ...
            if type(location) == tuple or type(location) == list:
                r = requests.get(self.url_base + "/commonapi/staticmap/getStaticImage",
